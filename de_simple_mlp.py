#code chuan cuối cùng
import torch
import torch.nn as nn
import torch.nn.functional as F


class DE_SimplE_MLP(nn.Module):
    def __init__(self, dataset, params):
        super(DE_SimplE_MLP, self).__init__()
        ...
        self.time_mlp = nn.Sequential(
            nn.Linear(3, params.t_emb_dim),
            nn.ReLU(),
            nn.Linear(params.t_emb_dim, params.t_emb_dim)
        ).to(self.device)
    ...
    def get_time_embedd(self, entities, years, months, days, h_or_t):
        ...
        # ===== 🔥 MLP TIME =====
        y = years.float() / 3000.0
        m = months.float() / 12.0
        d = days.float() / 31.0

        time_input = torch.cat([y, m, d], dim=1)
        time_mlp = self.time_mlp(time_input)

        # ===== 🔥 COMBINE =====
        tau = emb + 0.5 * time_mlp   # scale để ổn định

        return tau

   
    def getEmbeddings(self, heads, rels, tails, years, months, days):
    ...
    def forward(self, heads, rels, tails, years, months, days):
        ...
        h_embs1, r_embs1, t_embs1, h_embs2, r_embs2, t_embs2 = \
            self.getEmbeddings(heads, rels, tails, years, months, days)

        scores = ((h_embs1 * r_embs1) * t_embs1 +
                  (h_embs2 * r_embs2) * t_embs2) / 2.0
